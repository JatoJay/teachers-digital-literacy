...
type AssessmentNavState = { prefill?: AssessmentPrefill } | null

export default function AssessmentsPage() {
  const location = useLocation()
  const navigate = useNavigate()
  const { generate, isReady } = useModel()
  const navStateOnMount = useState<AssessmentNavState>(
    () => (location.state as AssessmentNavState) ?? null,
  )[0]
          ...
          <Card key={assessment.id} delay={index}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                {assessment.lessonId && lessonTitleById.has(assessment.lessonId) && (
                  <button
                    type="button"
                    onClick={() =>
                  ...
                  >
                    <BookOpen size={12} />
                    From: {lessonTitleById.get(assessment.lessonId)}
                  </button>
                )}
              </div>
